...
Diseño orientado a RAG: la cadena recibe el prompt ya construido. En el futuro,
para añadir RAG bastará insertar un paso previo (retriever) que recupere
solicitudes históricas o normativa universitaria y la inyecte como contexto
en el prompt, sin reescribir el resto del pipeline.
"""
...
def get_clasificacion_chain() -> Runnable:
    ...
    Pipeline:
        prompt | llm_estructurado    -> ClasificacionResult
                                     -> {'clasificacion': ...}
    ...
    # Subcadena: prompt -> LLM. El resultado se etiqueta como 'clasificacion'.
    clasificador: Runnable = (
        prompt | llm_estructurado | RunnableLambda(lambda r: {"clasificacion": r})
    )
